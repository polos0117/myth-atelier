...
(function (root) {
  ...
  /* ── 판의 신화권 — 다섯 권 중 셋만 그 판의 상점과 상대에 나온다. 카탈로그가 커져도 한 카드가 나오는 몫이
     줄지 않고(3성이 닿는 거리로), 판마다 구성이 달라진다. 저장된 옛 판(myths 없음)은 전부를 쓴다 ── */
  var MYTHS_PER_GAME = 3;
  function mythKeys(data) { return Object.keys(data.synergy.myth); }
  function inPlay(st, data) {
    if (!st.myths) return data.cards;
    return data.cards.filter(function (c) { return st.myths.indexOf(c.myth) >= 0; });
  }
  ...
  function makeEnemy(st, data) {
    ...
    var myths = st.myths || mythKeys(data), myth = myths[randInt(st, myths.length)];
  ...
  function fighter(data, u, side, row, col, id) {
    ...
    return { id: id, name: u.name, side: side, star: u.star, row: row, col: col,
             ...
             bonus: { atk: 0, spd: 0, manaRate: 0, skillPower: 0, splash: 0, stunChance: 0, firstcrit: 0, revive: 0, frenzy: 0 },
             alive: true, dealt: 0 };
  }
  ...
  var api = {
    ...
    MYTHS_PER_GAME: MYTHS_PER_GAME, inPlay: inPlay, setMyths: setMyths,

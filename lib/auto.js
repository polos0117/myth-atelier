...
(function (root) {
  ...
  var BENCH = 6, SHOP = 5, REROLL = 2, MAX_HP = 20, ROUNDS = 10, START_GOLD = 3;
  ...
  function roll(st, data) {
    var out = [], w = odds(st.round), i, r, cost, pool, j;
    for (i = 0; i < SHOP; i++) {
      r = rand(st) * 100; cost = 1;
      for (j = 0; j < 5; j++) { r -= w[j]; if (r < 0) { cost = j + 1; break; } }
      pool = data.cards.filter(function (c) { return c.cost === cost; });
  ...
  function newGame(data, seed) {
    var st = { seed: seed | 0, rngState: seed | 0, round: 1, gold: START_GOLD, hp: MAX_HP,
               shop: [], bench: [], board: [], nextId: 1, phase: 'plan', result: null, history: [], enemy: null };
  ...
  function buy(st, data, slot) {
    ...
    if (i === BENCH) return { ok: false, why: 'bench' };
    st.gold -= card.cost;
  ...
  function budget(round) { return Math.round(2 + round * 3.3); }
  function makeEnemy(st, data) {
    var round = st.round, n = Math.min(CELLS, 2 + Math.floor(round / 2));
    var maxCost = Math.min(5, 1 + Math.floor(round / 2.5));
    var minCost = Math.max(1, maxCost - 1);
    var myths = Object.keys(data.synergy.myth), myth = myths[randInt(st, myths.length)];
    var pool = data.cards.filter(function (c) { return c.cost >= minCost && c.cost <= maxCost; });
  ...
  function fighter(data, u, side, row, col, id) {
    ...
    return { id: id, name: u.name, side: side, star: u.star, row: row, col: col,
             ...
             bonus: { atk: 0, spd: 0, manaRate: 0, skillPower: 0, splash: 0, stunChance: 0, firstcrit: 0, revive: 0 },
             alive: true, dealt: 0 };
  }
  ...
  function cast(sim, e, t) {
    ...
    var mates = alive(sim.ents, e.side), power = 1 + e.bonus.skillPower, base = atkOf(e, t);
    ...
    switch (sk.effect) {
      ...
      case 'drain':
        ...
        for (i = 0; i < (sk.hits || 1); i++) {
          ...
          if (sk.effect === 'drain') { e.hp = Math.min(e.maxHp, e.hp + Math.round(real * sk.ratio)); sim.log.push({ t: +t.toFixed(1), k: 'heal', b: e.id, d: Math.round(real * sk.ratio) }); }
          if (!tgt.alive) break;
        }
  ...
  function simulate(data, st, mine, theirs) {
    ...
    return { winner: winner, beats: t, log: sim.log,
             ...
             ents: ents.map(function (e) { return { id: e.id, name: e.name, side: e.side, star: e.star, row: e.row, col: e.col,
               hp: e.hp, maxHp: e.maxHp, alive: e.alive, dealt: e.dealt }; }) };
  }

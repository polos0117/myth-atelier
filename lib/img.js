...
(function (root) {
  ...
  /* ── 신전 — 메인 화면의 신전에 도는 무기. 신전(테마)마다 따로 열둘, 도감에서 모시고 내린다.
     비어 있으면 메인이 그 신전의 신화권(group.json 의 temple)에서 무작위로 고른다.
     저장은 {테마: [이름…]}. 처음 판(배열 하나)은 만신전 몫으로 읽는다 ── */
  var HALL_STORE = 'myth_hall_v1', HALL_MAX = 12;
  function hallTheme(theme) {
    return theme || (root.AtelierAppearance && root.AtelierAppearance.get().theme) || 'pantheon';
  }
  function hallAll() {
    try {
      var j = JSON.parse(localStorage.getItem(HALL_STORE));
      if (Array.isArray(j)) return { pantheon: j };
      return j && typeof j === 'object' ? j : {};
    } catch (e) { return {}; }
  }
  function hallPicks(theme) { var l = hallAll()[hallTheme(theme)]; return Array.isArray(l) ? l.slice(0, HALL_MAX) : []; }
  function setHallPicks(list, theme) {
    var all = hallAll(); all[hallTheme(theme)] = (list || []).slice(0, HALL_MAX);
    try { localStorage.setItem(HALL_STORE, JSON.stringify(all)); } catch (e) {}
  }
  /* 모시기·내리기. 찼으면 {ok:false, why:'full'} */
  function toggleHall(name, theme) {
    var p = hallPicks(theme), i = p.indexOf(name);
    if (i >= 0) { p.splice(i, 1); setHallPicks(p, theme); return { ok: true, on: false }; }
    if (p.length >= HALL_MAX) return { ok: false, why: 'full' };
    p.push(name); setHallPicks(p, theme); return { ok: true, on: true };
  }
  ...
  root.AtelierImg = {
    ...
    HALL_MAX: HALL_MAX, hallTheme: hallTheme, hallPicks: hallPicks, setHallPicks: setHallPicks, toggleHall: toggleHall, hallShotsOf: hallShotsOf,

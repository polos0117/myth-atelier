/* 그림 자료 공통 계층 — 도감·오토 배틀·생성기가 같이 쓴다.
   ...
   img.json 구조:
     ...
     <몫> = {"f": "파일",                      액션 한 장(카드 그림)
             ...
             "casual": {"f": ["파일", …]},      일상컷 — 선택
             "skin": {"f": {"<열쇠>": "파일"}}}  스킨 — 선택. 열쇠는 카드 자료의 skins
...
(function (root) {
  ...
  function shotsOf(bucket, v) {
    ...
    var sk = skinsOf(bucket, v), skins = [], key;
    for (key in sk) skins.push(sk[key]);
  ...
  /* ── 스킨 — 정식 모습 위에 입히는 치장. 그림은 기본 한 장뿐이고 각성·저주는 화면이 효과로 덮는다 ── */
  function skinsOf(bucket, v) { var b = bucket && bucket.skin; return (b && b[v || 'f']) || {}; }
  /* 카드가 가진 스킨 그림 {열쇠: 파일}. 화풍을 가리지 않는다 — 한 스킨은 한 장이면 된다 */
  function skinFiles(entry, v) {
    var bs = styleMap(entry), out = {}, k, key, box;
    for (k in bs) { box = skinsOf(bs[k], v); for (key in box) if (!out[key]) out[key] = box[key]; }
    return out;
  }
  ...
  /* 판에 올릴 그림 — 고른 스킨이 있으면 {file, skin: 열쇠}, 없으면 상태 그림 {file, skin: null} */
  function dressOf(entry, name, v, star, cursed) {
    var key = skinPick(name), f = key && skinFiles(entry, v)[key];
    if (f) return { file: f, skin: key };
    return { file: coverOf(entry, v, undefined, star, cursed), skin: null };
  }
  ...
  root.AtelierImg = {
    ...
    skinsOf: skinsOf, skinFiles: skinFiles, skinPick: skinPick, setSkinPick: setSkinPick, dressOf: dressOf,
    load: load
  };
})(window);

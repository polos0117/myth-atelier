/* 프롬프트 조립 — 표(lib/prompt-spec.js)와 카드 자료를 받아 영어 프롬프트 한 덩이를 만든다.
   ...
   설정(st): {card, style, output(portrait|awaken|cursed|casual), design('mecha'|'cloth'), extras:{embodiment,silhouette,myth}, action(''=없음),
...
(function (root) {
  ...
  function build(st, data) {
    ...
    var awaken = st.output === 'awaken' || st.output === 'cursed', extras = st.extras || {}, lines = [];
  ...
  var api = { build: build, casualLines: casualLines, fileName: fileName, figureURL: figureURL, styleRow: styleRow, actionRow: actionRow,

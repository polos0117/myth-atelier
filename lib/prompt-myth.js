/* 프롬프트 조립 — 표(lib/prompt-spec.js)와 카드 자료를 받아 영어 프롬프트 한 덩이를 만든다.
   ...
   설정(st): {card, style, output(portrait|awaken|cursed|casual|skin), skin(스킨 열쇠), design('mecha'|'cloth'), extras:{embodiment,silhouette,myth}, action(''=없음),
...
(function (root) {
  ...
  function fileName(st, card) {
    ...
    return tpl.replace('{name}', card.name.replace(/ /g, '_')).replace('{style}', st.style || spec().DEFAULT_STYLE)
      .replace('{n}', String(st.casualIndex || 1)).replace('{skin}', sk ? sk.key : 'key');
  }
  ...
  var api = { build: build, casualLines: casualLines, fileName: fileName, skinOf: skinOf, figureURL: figureURL, styleRow: styleRow, actionRow: actionRow,

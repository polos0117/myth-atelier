/* 프롬프트 표 — 생성기가 쓰는 문장과 선택지는 전부 여기 있다. 함수는 없다(tests/words.cjs 가 본다).
   ...
   2026-09-21 방향 전환: "무기를 든 사람" 이 아니라 "무기가 사람이 된 것". 무기의 재질·색·형태·문양(카드의 look)이
   몸과 옷으로 옮겨 가고 실물 무기는 소품이 아니라 몸의 일부다. 첫 그림(궁니르)이 창 든 전사로 나와서 바꿨다.
...
(function (root) {
  ...
  /* 놀이 규칙 — 어느 출력에나 붙는다. 첫째가 가장 중요하다: 무기를 든 사람이 아니라 무기가 사람이 된 것 */
  var RULES = [
    'SUBJECT: one adult woman who IS the named legendary weapon given human form. She is not a warrior holding it and not its legendary wielder; she is the weapon itself, personified.',
    'EMBODIMENT: the weapon\u2019s materials, its two colors, its shape and its motifs (see WEAPON LOOK) must be carried by her body, hair, ornaments and costume \u2014 blade steel becomes plate and jewelry, haft wood becomes leather and braid, its glow becomes her eyes and hair tips, its engraving becomes her patterns. Her silhouette echoes the weapon\u2019s silhouette. The physical weapon may appear as a part of her (fused to a limb, floating at her back, worn as regalia) but never as a separately held prop; a viewer must recognize the weapon from her design alone even if the object were removed.',
    'ONE FIGURE ONLY. No second character, no creature, no text, no logo, no watermark, no speech bubble, no frame border.',
    'ANATOMY: natural adult proportions, both hands drawn correctly. No body-part enlargement, no extra limbs, no weapon-for-limb replacement that breaks the human figure.'
  ];

  /* 무기 종류별 실루엣 — 몸과 옷의 큰 방향 */
  var KIND_SILHOUETTE = {
    sword: 'SILHOUETTE: sword \u2014 sharp, straight and vertical; narrow shoulders-to-hips line, pointed hems and hair ends, edges everywhere, nothing rounded.',
    spear: 'SILHOUETTE: spear \u2014 tall and long-limbed, everything reads as one vertical line; narrow ornaments that point upward, a long straight fall of hair or cloth.',
    blunt: 'SILHOUETTE: blunt weapon \u2014 weight and mass; broad shoulders or heavy sleeves, dense rounded plates at fists, shoulders and hips, a grounded stance, thick braids.',
    bow: 'SILHOUETTE: bow \u2014 tension and curve; a lean drawn-bowstring posture, arcs in hair, sash and hem, a string-like line running through the design, quiver or arrows worn on the body.',
  ...
  /* 무기 종류별 자세 — 액션 한 장에서 고른다. 무기를 휘두르는 컷이 아니라 무기의 성격이 몸으로 드러나는 순간. 첫 것이 기본 */
  var ACTIONS = {
    sword: [
  ...
  root.AtelierSpec = {
    ...
    PARAM_GROUPS: PARAM_GROUPS, PARAMS: PARAMS, STYLE_SAMPLE: STYLE_SAMPLE
  };

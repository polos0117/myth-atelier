/* 던전 — 로그라이크 덱빌딩. 무기 셋을 동료로 데려가 여덟 칸의 길을 간다.
   카드는 무기가 준다(종류 공통 셋 + 고유 기술 하나). 각성은 한 싸움에서 그 무기 카드를 넉 장 쓰면
...
(function (root) {
  ...
  function member(data, name) {
    var c = cardOf(data, name), hp = Math.round(c.hp / tune(data).partyHpDiv);
    return { name: name, hp: hp, maxHp: hp, alive: true, awakened: false, cursed: false, offered: false, shield: 0, buff: 0 };
  }
  ...
  function newRun(data, seed, names) {
    if (!names || names.length !== 3) throw new Error('party');
    var st = { seed: seed | 0, rngState: seed | 0, node: -1, phase: 'map', battle: null, log: [], party: [] };
    names.forEach(function (n) { st.party.push(member(data, n)); });
    return st;
  }
  ...
  /* ── 상대 ── */
  function enemy(data, name, node, boss) {
    var c = cardOf(data, name), T = tune(data);
    var hp = Math.round(c.hp / T.enemyHpDiv * (1 + T.enemyHpPerNode * node) * (boss ? T.bossHp : 1));
    var atk = Math.round(c.atk * T.enemyAtk * (1 + T.enemyAtkPerNode * node) * (boss ? T.bossAtk : 1));
    return { name: name, hp: hp, maxHp: hp, atk: atk, skill: c.skill, alive: true, stunned: false, acts: 0, boss: !!boss };
  }
  function pickEnemies(st, data, boss) {
    var node = st.node, maxCost = boss ? 5 : Math.min(5, 1 + Math.floor(node / 2)), minCost = boss ? 5 : Math.max(1, maxCost - 1);
  ...
  function cardsFor(data, m) {
    var c = cardOf(data, m.name), list = data.run.cards[c.kind].map(function (k) {
      return { key: k.key, owner: m.name, cost: k.cost };
    });
    list.push({ key: 'skill:' + c.skill, owner: m.name, cost: 2 });
    return list;
  }
  ...
  function startBattle(st, data, boss) {
    var deck = [], id = 1;
    living(st).forEach(function (m) { m.shield = 0; m.buff = 0; cardsFor(data, m).forEach(function (c) { c.id = id++; deck.push(c); }); });
  ...
  function mult(st, data, m) {
    var c = cardOf(data, m.name), x = 1 + (m.buff || 0) + (st.battle.rally || 0);
  ...
  function play(st, data, handIndex, targetIndex) {
    ...
    if (!b.awake[m.name]) {
      b.gauge[m.name] += 1;
      if (b.gauge[m.name] >= T.gauge) { b.awake[m.name] = true; st.log.push({ k: 'awaken', a: m.name }); }
    }
    if (m.hp === 0) fall(st, m);
  ...
  function endTurn(st, data) {
    ...
    draw(st, data, T.hand);
    /* 저주를 묻는다 — 한 동료에게 한 번만 */
    var low = living(st).filter(function (p) { return !p.cursed && !p.offered && p.hp / p.maxHp < T.curseAt; })[0];
    if (low) { low.offered = true; b.ask = low.name; st.log.push({ k: 'ask', a: low.name }); }
  ...
  var api = { newRun: newRun, proceed: proceed, play: play, endTurn: endTurn, answerCurse: answerCurse,
    chooseShrine: chooseShrine, chooseGate: chooseGate, rest: rest, spec: spec, cardsFor: cardsFor, stateOf: stateOf,

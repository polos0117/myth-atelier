...
export function WorkspaceHeader({page}) {
  ...
  const title={index:'app.title',dex:'dex.title',auto:'auto.title',run:'run.title',prompt:'prompt.title'}[page];
  return h('div',{class:'workspace-header'+(folded?' folded':'')},
    h(WorkspaceHeading,{title:W(title),code:W('app.code'),
      subtitle:W(page==='prompt'?'prompt.subtitle':page==='run'?'run.subtitle':'app.subtitle')}),
    h('div',{class:'workspace-nav-row'},
      h('nav',{class:'workspace-nav'},['index','dex','auto','run','prompt'].map(key=>

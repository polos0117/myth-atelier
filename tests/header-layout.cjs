...
const PAGES=[['dex.html','.grid .cell'],['auto.html','.at-shop'],['run.html','.rn-pick'],['draft.html','.df-setup'],['prompt.html','#prompt-output'],['index.html','.theme-card']];
   ...
   try{
    for(const density of ['compact','relaxed']){
     ...
     for(const [file,ready] of PAGES){
      ...
      const scroll=p.locator(file==='dex.html'||file==='auto.html'||file==='run.html'||file==='draft.html'?'.collection-scroll':'.wrap');

...
const themes=['midnight','daylight','blossom','moss','plum','sand','deep','ember','stone','dancheong','pantheon'];
...
assert.equal(shapes.length,themes.length);
assert.equal(new Set(shapes).size,themes.length,'distinct motifs for every temple');
...
console.log('PASS: '+themes.length+' armory themes, persistent keys, palette previews, browser chrome and viewport sizing');
